...
test.describe('[QE-321] Verify section heading Score the Right Membership for You is displayed above comparison tiles', () => {
  let scorecardPage;
  ...
  test('[QE-1][AC9] Verify section heading is displayed above comparison tiles', async ({ page }) => {
    ...
    await test.step('Navigate to the comparison section', async () => {
      ...
      const comparisonSectionVisible = await scorecardPage.isComparisonSectionVisible();
      expect(comparisonSectionVisible).toBeTruthy();
    });
    ...
    await test.step('Verify heading is positioned above the comparison tiles', async () => {
      const sectionHeading = loc.sectionHeading(page);
      const scorecardTile = loc.scorecardTile(page);
      const scorecardPlusTile = loc.scorecardPlusTile(page);
