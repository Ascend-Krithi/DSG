const { test, expect } = require('../../fixtures');
const ScorecardPage = require('../../pages/scorecard.page');

test.describe('[QS-107][TS-005] Scorecard+ Dark Logo Quality and Accessibility Tests', () => {
  let scorecardPage;

  test.beforeEach(async ({ page }) => {
    scorecardPage = new ScorecardPage(page);
    await page.setViewportSize({ width: 1920, height: 1080 });
    await scorecardPage.goto();
  });

  test('[QS-205][TC-015] Verify Scorecard+ Dark Logo renders correctly with proper contrast and quality', async ({ page }) => {
    // Locate Scorecard+ Dark Logo on tile
    const scorecardPlusLogo = page.getByAltText('ScoreCard Plus New Logo');
    await expect(scorecardPlusLogo).toBeVisible();

    // Verify logo image quality
    const logoBox = await scorecardPlusLogo.boundingBox();
    expect(logoBox).not.toBeNull();
    expect(logoBox.width).toBeGreaterThan(50);
    expect(logoBox.height).toBeGreaterThan(20);

    // Verify logo has alt text attribute
    const altText = await scorecardPlusLogo.getAttribute('alt');
    expect(altText).toBeTruthy();
    expect(altText).toContain('ScoreCard');

    // Zoom page to 125%
    await page.evaluate(() => {
      document.body.style.zoom = '125%';
    });
    await page.waitForTimeout(500);
    
    await expect(scorecardPlusLogo).toBeVisible();
    const logoBox125 = await scorecardPlusLogo.boundingBox();
    expect(logoBox125).not.toBeNull();

    // Zoom page to 150%
    await page.evaluate(() => {
      document.body.style.zoom = '150%';
    });
    await page.waitForTimeout(500);
    
    await expect(scorecardPlusLogo).toBeVisible();
    const logoBox150 = await scorecardPlusLogo.boundingBox();
    expect(logoBox150).not.toBeNull();

    // Reset zoom
    await page.evaluate(() => {
      document.body.style.zoom = '100%';
    });
  });
});